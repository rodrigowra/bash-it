...
#PS1="$G┌──[$BB\u$G]───[$BR$(date "+%a,%d %b %y")$G]───[$BB\t$G]───[$Y\w$G]───[$BM$(free | awk '/Mem/{printf("Memory:%d mb"), $3/(2^10)} /Swap/{printf(",swap:%d "), $3/(2^100)}')$G]───┐ \n└──>>>$G "
# ⓡⓞⓓⓡⓘⓖⓞⓦⓡⓐ
    # PS1="${green}┌[${blue}ⓡ ⓞ ⓓ ⓡ ⓘ ⓖ ⓞ ${green}]─[${red}$(free | awk '/Mem/{printf("Memory:%d mb"), $3/(2^10)} /Swap/{printf(",swap:%d"),$3/(2^100)}')${green}]─[${dir_color}\\w${green}]─${purple}$(scm_char)$(scm_prompt_info)${green}─┐ \n└──▶ "

    PS1="${green}┌─[${blue}\u${green}]─[${purple}$(date "+%a,%d %b")${green}]─[${red}$(free | awk '/Mem/{printf("Memory:%d mb"), $3/(2^10)} /Swap/{printf(",swap:%d "), $3/(2^100)}')${green}]─[${dir_color}\\w${green}]─${purple}$(scm_char)$(scm_prompt_info)${green}─┐ \n└──>>> "
